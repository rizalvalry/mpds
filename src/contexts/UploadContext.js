      ...
      const result = await chunkedUploadService.uploadBatch(
        ...
        (fileId, progress, fileName) => {
          ...
          if (fileIndex !== -1) {
            ...
            // Update completed files list for retry mechanism AND realtime stats
            if (progress >= 100 && pendingUploadRef.current) {
              ...
              if (!completedIds.includes(fileId)) {
                ...
                // FIX: Update uploadStats in realtime as each file completes
                // This fixes the 0/319 counter issue in GlobalUploadIndicator
                setUploadStats(prev => ({
                  ...prev,
