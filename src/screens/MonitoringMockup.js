              ...
              {panelsData.completed.length === 0 ? (
                ...
                panelsData.completed.map((area, index) => (
                  <View key={index} style={styles.blockCard}>
                    ...
                    {/* Timestamps for COMPLETED panel - shows upload & detection timeline */}
                    <View style={styles.timestampContainer}>
                      {/* Upload Timestamps */}
                      <View style={styles.timestampRow}>
                        <Text style={styles.timestampLabel}>📤 Upload Start:</Text>
                        <Text style={styles.timestampValue}>
                          {area.createdAt
                      ...
                      {/* Detection Timestamps */}
                      <View style={styles.timestampRow}>
                        <Text style={styles.timestampLabel}>🚀 Detection Start:</Text>
                        <Text style={styles.timestampValue}>
                          {area.detectionStartedAt
                            ? area.detectionStartedAt.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })
                            : '-'}
                      ...
                      <View style={styles.timestampRow}>
                        <Text style={styles.timestampLabel}>✅ Detection End:</Text>
                        <Text style={styles.timestampValue}>
                          {area.detectionCompletedAt
                            ? area.detectionCompletedAt.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })
                            : '-'}
                      ...
                      {/* Duration calculation if both timestamps exist */}
                      {area.detectionStartedAt && area.detectionCompletedAt && (
                        <View style={[styles.timestampRow, { marginTop: 4, borderTopWidth: 1, borderTopColor: '#E2E8F0', paddingTop: 4 }]}>
                          <Text style={[styles.timestampLabel, { fontWeight: '600' }]}>⏱️ Duration:</Text>
                          <Text style={[styles.timestampValue, { fontWeight: '700', color: '#10B981' }]}>
                            {(() => {
                              const durationMs = area.detectionCompletedAt - area.detectionStartedAt;
